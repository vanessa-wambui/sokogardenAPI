...
# configure the loation to where your product images will be saved
...
def signin():
    if request.method == "POST":
        ...
        # if there are records returned the password and email are correct otherwise it means they are wrong
        if count == 0:
            return jsonify({"message" : "login failed"})
        else:
            # there must be a user we create a variable that will hold the details of user fetched frm database
            user=cursor.fetchone()
            # return the details toe frontend as well as a message
...
# below is the route for product 
@app.route("/api/addproducts",methods=["POST"])
def addproducts():
    if request.method == "POST":
        # extract the data in the form
        product_name = request.form["product_name"]
        product_description = request.form["product_description"]
        product_cost = request.form["product_cost"]
        # for the product photo it is fetched from files as shown below
        ...
        # save the product photo image into yhe new locaion
        ...
        # structure the sql query to insert the products details
        sql= "INSERT INTO product_details(product_name, product_description, product_cost, product_photo) VALUES (%s, %s, %s, %s)"
        # crete a tuple that will hold the data which are currntly held in variablea declared
        data = (product_name, product_description, product_cost, filename)

        # use the cursor to execute t
        cursor.execute(sql,data)
